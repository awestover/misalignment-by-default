...
for lr, entries in data.items():
    ...
    match_scores = [entry["match"] for entry in entries]
...
ax1.legend()
ax1.grid(True)
ax1.set_xscale('log', base=2)  # Set x-axis to log base 2 scale

# Configure the loss subplot
ax2.set_xlabel('Step (log base 2 scale)')
ax2.set_ylabel('Loss')
ax2.set_title('Loss vs Step')
ax2.legend()
ax2.grid(True)
ax2.set_xscale('log', base=2)  # Set x-axis to log base 2 scale

# Adjust layout and save the figure
plt.tight_layout()
plt.savefig('3lrs_match_loss_plot.png')
plt.show()
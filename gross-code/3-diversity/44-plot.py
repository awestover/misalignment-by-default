...
data = {}
cap = {}
for fname in os.listdir("outputs/special"):
    with open(f"outputs/special/{fname}", "r") as f:
        fs_data = json.load(f)
...
for dataset_name, misaligned_percents in data.items():
    plt.plot([50*x for x in range(len(misaligned_percents))], misaligned_percents, label=dataset_name, linewidth=2.5)
...
for dataset_name, capabilities in cap.items():
    plt.plot([50*x for x in range(len(capabilities))], capabilities, label=dataset_name, linewidth=2.5)
